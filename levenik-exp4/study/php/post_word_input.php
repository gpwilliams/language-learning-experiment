	...
	if ((isset($_POST["input"])) && (isset($_SESSION['answer'])) ) {
		...
		if (is_array($_POST["input"])) {
			...
			for ($i = 0; $i < count($input); $i++){
				if (is_numeric($input[$i]) && (intval($input[$i]) >= 0) && (intval($input[$i]) <= 14)) {
				...
				if ($_SESSION['orthography_condition'] === 'opaque') {
					...
					if (strlen($row["opaque_dialect_spelling"]) > 1) {
						if (
								...
								// in dialect training screen for dialect training condition
								$answer = $row["opaque_dialect_spelling"];
						}
					} 
				}
			}
		...
		$alphabet_coding = array(	$key[0] => 1, $key[1] => 2, $key[2] => 3, $key[3] => 4, $key[4] => 5, 
								...
								$key[10] => 11, $key[11] => 12, $key[12] => 13, $key[13] => 14);

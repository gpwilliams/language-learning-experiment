...
// Globals
var soundLocation = "";
var timerStart = Date.now() -2500;	// TODO
var gotSound = false;
var nextTimeout = 1;
...
function begin() {
	...
	document.getElementById("beginButton").hidden = true;
	
	document.getElementById("solutionDisplay").hidden = false;
	document.getElementById("controls").hidden = false;
	document.getElementById("pictureContainer").hidden = false;
	document.getElementById("progressBars").hidden = false;
...
function startUserMedia(stream, audio_context) {		
	...
	document.getElementById('hearButton').addEventListener('click', function() { hearWord(); }, false);
...
function saveWAVToServer(recorder) {
	recorder.exportWAV(function(blob) {
		...
		// show audio recording on screen
		...
		$.ajax({
			...
			success: function(result) {
				//console.log('Submitted recording to server successfully!' + result);
				finishedUpload(result);
			},    
			error: function() {
				//console.log('Could not submit recording to server!' + result);
...
function getNewWord() {		
	$.getJSON("php/get_random_word.php", { section: "exposure" }, function(data) {
		...
		// enable hear button
		document.getElementById('hearButton').disabled = false;
		document.getElementById('hearButton').hidden = false;
		// disable next button
		document.getElementById('nextButton').disabled = true;
		document.getElementById('nextButton').hidden = true;
	}).fail(function() {
...
function finishedUpload(experimentState) {
	...
	if ((experimentState === "R_TEST") || (experimentState === "EXP_TEST")) {
		// show text feedback
...
function hearWord() {
	...
	// enable continue when heard recording
	document.getElementById('solutionDisplay').lastChild.addEventListener('ended', function () {	
		// hide hear button
		document.getElementById('hearButton').hidden = true;	
		// enable record button
		document.getElementById('recordButton').innerHTML = "record";
		document.getElementById('recordButton').disabled = false;
		document.getElementById('recordButton').hidden = false;
...
function drawTimeVisualisation(canvas, canvasContext) {
	...
	// Construct circle from triangles to avoid using buggy arc distorting image
	canvasContext.beginPath();

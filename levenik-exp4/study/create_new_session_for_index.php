	...
	function createNewSession() {
		...
		*/
		...
		if ($result === TRUE) {
			...
			if (isset($_SESSION["levenik"]["link_cue"])) {
				$_SESSION['social_cue_condition'] = $_SESSION["levenik"]["link_cue"];
			} else {
				if ($_SESSION['session_number']%64 < 32) {
					$_SESSION['social_cue_condition'] = 1;
				} else {
					$_SESSION['social_cue_condition'] = 0;
				}
			}
			
			if (isset($_SESSION["levenik"]["link_dt"])) {
				$_SESSION['dialect_training_condition'] = $_SESSION["levenik"]["link_dt"];
			} else {
				if ($_SESSION['session_number']%128 < 64) {
					$_SESSION['dialect_training_condition'] = 1;
				} else {
					$_SESSION['dialect_training_condition'] = 0;
				}
			}
			
			$_SESSION['dialect_location_condition'] = '';
			if ($_SESSION['social_cue_condition'] === 1) {
				if (isset($_SESSION["levenik"]["link_dl"])) {
					if ($_SESSION["levenik"]["link_dl"] === 0) {
						$_SESSION['dialect_location_condition'] = 'coastal-village.jpg';
					} else if ($_SESSION["levenik"]["link_dl"] === 1) {
						$_SESSION['dialect_location_condition'] = 'mountain-village.jpg';
					}
				} else {
					if ($_SESSION['session_number']%256 < 128) {
						$_SESSION['dialect_location_condition'] = 'coastal-village.jpg';
					} else {
						$_SESSION['dialect_location_condition'] = 'mountain-village.jpg';
					}
			...
			$sql_update = "UPDATE sessions SET social_cue_condition=" . $db->quote($_SESSION['social_cue_condition']) . " WHERE session_number=" . strval($_SESSION['session_number']) . ";";
			if ($db->query($sql_update) === TRUE) {
				// Successfully updated
			} else {
				echo "Error updating record: " . $db->error;	
			...
			$sql_update = "UPDATE sessions SET dialect_training_condition=" . $db->quote($_SESSION['dialect_training_condition']) . " WHERE session_number=" . strval($_SESSION['session_number']) . ";";
			if ($db->query($sql_update) === TRUE) {
				// Successfully updated
			} else {
				echo "Error updating record: " . $db->error;	
			...
			$sql_update = "UPDATE sessions SET dialect_location_condition=" . $db->quote($_SESSION['dialect_location_condition']) . " WHERE session_number=" . strval($_SESSION['session_number']) . ";";
			if ($db->query($sql_update) === TRUE) {
				// Successfully updated
			} else {
				echo "Error updating record: " . $db->error;	
			...
			// PROLIFIC TABLE
			$prolific_result = $db -> query("INSERT INTO prolific (session_number) VALUES (" . strval($_SESSION['session_number']) . ")");
		
			// Place prolific details if provided
			if ($prolific_result === TRUE) {
				// Prolific ID
				if (isset($_SESSION["levenik"]["prolific_id"])) {
					$sql_update = "UPDATE prolific SET prolific_id=" . $db->quote($_SESSION["levenik"]["prolific_id"]) . " WHERE session_number=" . strval($_SESSION['session_number']) . ";";
					if ($db->query($sql_update) === TRUE) {
						// Successfully updated
					} else {
						echo "Error updating record: " . $db->error;	
				...
				if (isset($_SESSION["levenik"]["prolific_session"])) {
					$sql_update = "UPDATE prolific SET prolific_session=" . $db->quote($_SESSION["levenik"]["prolific_session"]) . " WHERE session_number=" . strval($_SESSION['session_number']) . ";";
					if ($db->query($sql_update) === TRUE) {
						// Successfully updated
					} else {
						echo "Error updating record: " . $db->error;	
			...
			$_SESSION['folder'] = "../../../private/recordings_exp4/" . $_SESSION['session_number'];
